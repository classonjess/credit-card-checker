...
/*  If index + 2 is even, double the value. If above 9 then adjust the value*/

   for (let i = 0; i < arr.length; i ++){
     let curDigit  = parseInt(arr[i]);

     if ((i + 2) % 2 === 0)
       if ((curDigit *= 2) > 9)
            curDigit -= 9;
/* the above if it is above 9 */

     count += curDigit; /* Adds it all together */
     }
     if ((count % 10) === 0){
       return true;
     } else
...
const findInvalidCards = check => {
  let inv = [ ]; ///Set list of invalid cards.
...
const idInvalidCardCompanies = invalidNumbers => {
 ...
 for( i = 0; i < invalidNumbers.length; i++){
   if (invalidNumbers[i][0] === 6 ){
     companies.push("Discover");
    }else if ( invalidNumbers[i][0] === 3 ){
     companies.push(`Amex`);
